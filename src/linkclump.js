...
function start() {
  ...
  for (var i = 0; i < page_links.length; i++) {
    ...
    page_links[i].important = (this.settings[this.setting].options.smart === 0 &&
...
function detech(x, y, open) {
  ...
  if (!this.scroll_id) {
    this.scroll_id = setInterval(scroll.bind(this), 100);
  }
  ...
  for (var i = 0; i < this.links.length; i++) {
    if (
      ...
      !(this.links[i].x1 > this.box.x2 || this.links[i].x2 < this.box.x1 || this.links[i].y1 > this.box.y2 || this.links[i].y2 < this.box.y1)
    ) {
      if (open) {
        open_tabs.push({
